...
def run_bogu_consumer_loop():
    ...
    try:
        consumer = KafkaConsumer(
            ...
            group_id='bogu-ai-group',
            api_version=(0, 11, 0)
        )
        logger.info("✅ Bogu AI 소비자 루프 시작 (그룹: bogu-ai-group)")
        ...
        logger.info("✅ Bogu AI Kafka Producer 초기화 완료")

        for message in consumer:
            try:
                # Kafka headers는 bytes이므로 문자열로 디코딩
                headers_dict = {
                ...
                if message_type == "BoguAiRequest":
                    ...
                    response_headers = [('type', b'BoguDocumentCreated')]

                    producer.send(KAFKA_TOPIC, value=response_payload, headers=response_headers)
                    producer.flush()

                    logger.info(f"📤 Bogu AI 응답 전송 완료 → ID: {original_request_id}")
                else:
        ...
        if consumer:
            consumer.close()
            logger.info("🛑 Bogu AI 소비자 종료")
        if producer:
            producer.close()
            logger.info("🛑 Bogu AI Producer 종료")